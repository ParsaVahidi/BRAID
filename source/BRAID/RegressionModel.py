...
import os, time, copy, re, io, logging
...
from .tools.tf_losses import masked_R2, masked_CC, masked_mse, masked_PoissonLL_loss, masked_CategoricalCrossentropy
...
class RegressionModel(tf.keras.layers.Layer, ModelWithFitWithRetry, Reconstructable):
    ...
    """    
    def __init__(self, n_in, n_out, units=[], 
                        use_bias=False, 
    ...
    def build(self):
        ...
        self.inputs = tf.keras.Input(shape=(self.n_in,), name='{}input'.format(self.name_prefix))
        x = self.inputs
        for ci in range(self.num_layers):
            ...
            if kernel_regularizer is not None:
                ...
                kernel_regularizer = getattr(tf.keras.regularizers, kernel_regularizer)(**args)
            ...
            if bias_regularizer is not None:
                ...
                bias_regularizer = getattr(tf.keras.regularizers, bias_regularizer)(**args)
            nUnits = copy.copy(self.units[ci])
            if self.num_classes is not None and ci == (self.num_layers-1): # Output layer for classification
            ...
            if this_dropout_rate > 0 and (ci < (self.num_layers-1) or len(self.dropout_rate) >= self.num_layers):
                dropout = tf.keras.layers.Dropout(this_dropout_rate, name='{}dropout_{}'.format(self.name_prefix, ci+1))
        ...
        if self.has_prior_pred:
            if self.num_classes is not None:
                self.prior_pred = tf.keras.Input(shape=(self.n_out,self.num_classes,), name='{}prior_pred'.format(self.name_prefix))
            else:
                self.prior_pred = tf.keras.Input(shape=(self.n_out,), name='{}prior_pred'.format(self.name_prefix))
            if self.prior_pred_op == 'add':
                self.outputs = self.outputs + self.prior_pred  # Here we have either logits or Gaussian vars so they add up in both cases
            elif self.prior_pred_op == 'multiply':
                self.outputs = self.outputs * self.prior_pred  # Useful for outdist=poisson with exponential activation function
            else:
                raise(Exception("Not supported"))
            self.model = tf.keras.models.Model(inputs=[self.inputs, self.prior_pred], outputs=self.outputs)
        else:
    ...
    def apply_func(self, inputs, name_scope=None):
        with tf.name_scope(self.name_prefix if name_scope is None else name_scope) as scope:
            if self.has_prior_pred:
                out = inputs[0]
                prior_pred = inputs[1]
            else:
            ...
            if self.has_prior_pred and prior_pred is not None:
                if self.missing_marker is not None:
                    mask_value_cast = tf.constant(self.missing_marker, dtype=prior_pred.dtype, name='missing_marker')
                    isOk = tf.not_equal(prior_pred, mask_value_cast)
                    hasPrior = tf.cast(tf.math.reduce_all(isOk, axis=1), dtype=prior_pred.dtype, name='prior_missing_mask') # Batch x 1, will be 1 for batches that have input
                else:
                    hasPrior = tf.ones_like(prior_pred, dtype=prior_pred.dtype, name='missing_mask')[:, 0] # Batch x 1
                hasPriorExpand = tf.expand_dims(hasPrior, axis=1, name='prior_not_missing')
                noPriorExpand = tf.identity(1-hasPriorExpand, name='missing')
                if self.prior_pred_op == 'add':
                    check_op = tf.Assert(~tf.math.reduce_all(prior_pred == 1), ['Prior is always exactly 1! Might be a bug in passing the prior!'], name='CHECK_NONONE')
                    out += tf.cast(prior_pred*hasPriorExpand, dtype=out.dtype) # Here we have either logits or Gaussian vars so they add up in both cases
                    # print(hasPriorExpand)
                elif self.prior_pred_op == 'multiply':
                    check_op = tf.Assert(~tf.math.reduce_any(prior_pred < 0), ['Prior is negative for poisson! Probably a bug in passing the prior!', prior_pred], name='CHECK_NONNEGATIVE')
                    check_op2 = tf.Assert(~tf.math.reduce_all(prior_pred == 0), ['Prior is always zero for poisson! Probably a bug in passing the prior!'], name='CHECK_NONZERO')
                    out *= tf.cast(prior_pred, dtype=out.dtype) # Useful for outdist=poisson with exponential activation function
                    raise(Exception('TO DO: add support for missing priors for the multiply op'))
                else:
                    raise(Exception("Not supported"))
            elif self.has_prior_pred and prior_pred is not None:
                print(f'prior_pred.shape = {prior_pred.shape}')# (missing count: {tf.math.reduce_sum(prior_pred==self.missing_marker)})')
        return out
    ...
    def fit(self, X_in, X_out, prior_pred=None, X_in_val=None, X_out_val=None, prior_pred_val=None, 
        ...
        def prep_IO_data(X_in, X_out, prior_pred, goal_label_for_log='training'):
            if self.missing_marker is not None:
                isNotMissing = np.logical_not(np.any(X_out==self.missing_marker, axis=0))
                if not np.all(isNotMissing):
                    logger.info('Only {}/{} samples ({:.3g}%) can be used for {} (rest is missing, i.e. marker:{})'.format(np.sum(isNotMissing), len(isNotMissing), 100*np.sum(isNotMissing)/len(isNotMissing), goal_label_for_log, self.missing_marker))
            else:
            ...
            if self.has_prior_pred:
                if prior_pred is None:
                    if self.num_classes is not None: # Default is equiprobable
                        prior_pred = np.ones((X_out.shape[0], X_out.shape[1], self.num_classes))/self.num_classes
        ...
        if X_in_val is None:
            ...
            inputs_val, outputs_val = prep_IO_data(X_in_val, X_out_val, prior_pred_val, goal_label_for_log='validation')
    ...
    def predict(self, X_in, prior_pred = None):
        ...
        if self.has_prior_pred and prior_pred is None:
            if self.num_classes is not None: # Default is equiprobable
                prior_pred = np.ones((self.n_out, X_in.shape[1], self.num_classes))/self.num_classes
            else:  # Default is zero
                prior_pred = np.zeros((self.n_out, X_in.shape[1]))
        if not hasattr(self.model, '_predict_counter'): # Temp to fix new Keras error when using old models to predict:
...
class DRModel(PredictorModel):
    ...
    @staticmethod
    def parse_method_code(methodCode, YType=None, ZType=None, Z=None, missing_marker=None):
        Dz_args={}
        Dy_args={}
        if 'HL' in methodCode:
    ...
    def fit(self, Y, Z=None, U=None, 
        ...
        model_Dz = RegressionModel(ny+nu, nz, log_dir=this_log_dir, missing_marker=missing_marker, **reg_args)
        history_Dz = model_Dz.fit(YU, Z, X_in_val=YU_validation, X_out_val=Z_validation, epochs=epochs)
    ...
    def restoreModels(self):
        if self.nz > 0:
            ...
            self.model_Dz = RegressionModel(self.ny+self.nu, self.nz, missing_marker=self.missing_marker, **reg_args)
