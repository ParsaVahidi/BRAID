...
from BRAID.tools import MatHelper as mh
...
from .tools.LSSM import genRandomGaussianNoise
...
def shift_ms_to_1s_series(Y, steps_ahead, missing_marker=None, time_first=True):
    ...
    for saInd, step_ahead in enumerate(steps_ahead):
        ...
        if step_ahead == 0:
            logger.warning(f'[TEMP] Ignoring shift for step_ahead=0 (treated exactly as step_ahead=1). Not expected for BRAID.')
            pred_shifted = pred
        elif step_ahead == 1:
            pred_shifted = pred
        elif step_ahead > 1:
...
def shift_1s_to_ms_series(Y_shifted_steps, steps_ahead, missing_marker=None, time_first=True):
    """Shifts multi step ahead predictions of the RNNModel from
        ====> time
        1-step: z^{0|-1}, z^{1| 0}, z^{2| 1}, z^{3| 2}, ...
        2-step:  nan    , z^{1|-1}, z^{2| 0}, z^{3| 1}, z^{4| 2}, ...
    ...
    To:
        1-step: z^{0|-1}, z^{1| 0}, z^{2| 1}, z^{3| 2}, ...
        2-step: z^{1|-1}, z^{2| 0}, z^{3| 1}, z^{4| 2}, ..., nan
        3-step: z^{2|-1}, z^{3| 0}, z^{4| 1}, z^{5| 2}, ..., nan    ,  nan
        4-step: z^{3|-1}, z^{4| 0}, z^{5| 1}, z^{6| 2}, ..., nan    ,  nan    ,  nan
    So that in each sample, the prediction given the same data, with different horizons into the future
    Args:
        Y_shifted_steps (list/tuple/np.array): predictions for all steps, or an np.array that is the prediction for one time step
        steps_ahead (list of int): list of steps ahead. if None will be taken as [1]
        missing_marker (np, optional): value that will be used to represent missing samples. Defaults to np.nan.
        time_first (bool, optional): if true, will take the first dim to be time. 
                If False will take the 2nd dim to be time. Defaults to True.
    ...
    Y = []
    for saInd, step_ahead in enumerate(steps_ahead):
        pred =  Y_shifted_steps[saInd] if isinstance(Y_shifted_steps, (list, tuple)) else Y_shifted_steps
        if step_ahead == 0:
            logger.warning(f'[TEMP] Ignoring shift for step_ahead=0 (treated exactly as step_ahead=1). Not expected for BRAID.')
            pred_shifted_back = pred
        elif step_ahead == 1:
            pred_shifted_back = pred
        elif step_ahead > 1:
            pred_shifted_back = missing_marker * np.ones_like(pred)
            if time_first:
                pred_shifted_back[:(-step_ahead+1), ...] = pred[(step_ahead-1):, ...]
            else:
                pred_shifted_back[:, :(-step_ahead+1),...] = pred[:, (step_ahead-1):, ...]
        else:
            raise(Exception('step_ahead must be >= 1'))
...
def MainModelPrepareArgs(methodCode):
    ...
    Args:
        methodCode (string): BRAID method code string. For example can be "BRAID_uAKCzCy2HL128U"
             for a fully nonlinear model with 2 hidden layers of 128 units for each model parameter.

    Returns:
        kwargs: dict of arguments for the MainModel constructor.
    """    
    A1_args={}
    K1_args={}
    Cy1_args={}
    ...
    Cz2_args={}
    if 'HL' in methodCode:
        regex = r"([A|K|Cy|Cz|A1|K1|Cy1|Cz1|A2|K2|Cy2|Cz2|]*)(\d+)HL(\d+)U" #Cz1HL64U
        ...
        activation = 'relu'
        NL_args = {'use_bias': True, 'units': [hidden_units]*hidden_layers, 'activation': activation}
    ...
    if 'OGInit' in methodCode: # Use initializers similar to the default keras LSTM initializers
    ...
    if 'RGL' in methodCode: # Regularize
        ...
        if norm_num is not None and norm_num != '': # Add L1 or L2 regularization
            ...
            RGL_args.update({
                    'kernel_regularizer_name': regularizer_name, 'kernel_regularizer_args': regularizer_args
                })
            if 'RGLB' in methodCode: # Also regularize biases
                    # Reference for why usually biases don't need regularization:
                    # http://neuralnetworksanddeeplearning.com/chap3.html
                RGL_args.update({
                        'bias_regularizer_name': regularizer_name, 'bias_regularizer_args': regularizer_args
                    })
        if var_names == '' or 'A1' in var_names or ('A' in var_names and 'A2' not in var_names):
    ...
    if 'ErS' in methodCode: # Early stopping
        ...
        early_stopping_patience = 3 # TEMP!!! changes from 3 to 32
        ErS_str = ''
    ...
    optimizer_args = None
    # learning_rates, matches = extractNumberFromRegex(methodCode, prefix='LR') # LR1e-05 or LR0.01
    ...
    steps_ahead, steps_ahead_loss_weights, matches = parseMethodCodeArgStepsAhead(methodCode)

    model1_Cy_Full = 'FCy' in methodCode or 'FCyCz' in methodCode or 'FCzCy' in methodCode
    model2_Cz_Full = 'FCz' in methodCode or 'FCyCz' in methodCode or 'FCzCy' in methodCode
    ...
    observable_U_in_Kfw = 'ObsUKfw' in methodCode
    observable_U_in_Cfw = 'ObsUCfw' in methodCode
    ...
    kwargs = {
        ...
        'observable_U_in_Kfw': observable_U_in_Kfw,
...
def MainModelSetTrainableParameters(base=None, fw=None, initial_state=None, 
    model1=None, model2=None, model1_Cy=None, model2_Cz=None, model1_Cy_fw=None, model2_Cz_fw=None):
...
class MainModel(PredictorModel):
    ...
    def add_default_param_args(self, 
            A1_args = {}, K1_args = {}, Cy1_args = {}, Cz1_args = {}, 
            A2_args = {}, K2_args = {}, Cy2_args = {}, Cz2_args = {}, 
            yDist = None, zDist = None):
        LinArgs = {'units': [], 'use_bias': False, 'activation': 'linear', 'output_activation': 'linear'}
        ...
        return A1_args, K1_args, Cy1_args, Cz1_args, A2_args, K2_args, Cy2_args, Cz2_args
    ...
    def get_model_steps_ahead(self, steps_ahead=None, steps_ahead_loss_weights=None):
        if steps_ahead is not None:
            ...
            steps_ahead_model1 = list(range(1,1+maxStepsAhead))
            model1_orig_step_inds = [np.where(np.array(steps_ahead_model1) == step_ahead)[0][0] for step_ahead in steps_ahead]
            steps_ahead_loss_weights_model1 = [0.0 for saInd in range(len(steps_ahead_model1))]
            for saInd in range(len(steps_ahead)):
                if steps_ahead_loss_weights is not None:
                    steps_ahead_loss_weights_model1[model1_orig_step_inds[saInd]] = steps_ahead_loss_weights[saInd]
                else:
                    steps_ahead_loss_weights_model1[model1_orig_step_inds[saInd]] = 1.0
        else:
            steps_ahead = [1] # Default is 1 step ahead
            steps_ahead_model1 = steps_ahead
            steps_ahead_loss_weights_model1 = steps_ahead_loss_weights
            model1_orig_step_inds = [0]
        return steps_ahead, steps_ahead_loss_weights, steps_ahead_model1, steps_ahead_loss_weights_model1, model1_orig_step_inds
    ...
    def prep_observation_for_training(self, Y, YType):
        ...
        if YType == 'cat':
            ...
            if Y is not None:
                ...
                YTrue = np.ones((Y.shape[1],Y.shape[0],len(YClasses)), dtype=int) * (int(self.missing_marker) if self.missing_marker is not None else -1)
            ...
            yDist = 'poisson'
            YLossFuncs = [
                masked_PoissonLL_loss(self.missing_marker)
            ]
            if Y is not None:
                YTrue = Y.T
        else:
    ...
    def finetune(self, Y, Z=None, U=None, 
            **kw_args
        ):
        """Finetunes a learned BRAID model by starting from that model and continuing optimization
        ...
        forced_defaults = {
            'nx': self.nx,
            'n1': self.n1,
            'batch_size': self.batch_size,    # If not none, will set the batch_size of the MainModel to this value
            ...
            'A_args': self.A_args, 'K_args': self.K_args, 'Cy_args': self.Cy_args, 'Cz_args': self.Cz_args,    # Both stage 1 and 2 params
            'A1_args': self.A1_args, 'K1_args': self.K1_args, 'Cy1_args': self.Cy1_args, 'Cz1_args': self.Cz1_args, # Stage 1 params
            'A2_args': self.A2_args, 'K2_args': self.K2_args, 'Cy2_args': self.Cy2_args, 'Cz2_args': self.Cz2_args, # Stage 2 params
            'optimizer_name': self.optimizer_name if hasattr(self, 'optimizer_name') else 'Adam', # Name of optimizer
            'optimizer_args': self.optimizer_args if hasattr(self, 'optimizer_args') else None, # Dict of arguments for the optimizer
            'lr_scheduler_name': self.lr_scheduler_name if hasattr(self, 'lr_scheduler_name') else None, # Name of learning rate scheduler
            'lr_scheduler_args': self.lr_scheduler_args if hasattr(self, 'lr_scheduler_args') else None, # Dict of arguments for the learning rate scheduler
        }
        kw_args.update(forced_defaults)
        # Soft defaults for which the value from kw_args (if available) will instead be used
        soft_defaults = {
            'enable_forward_pred': self.enable_forward_pred if hasattr(self, 'enable_forward_pred') else False,      # If True, will enable forward prediction by having a separate set of Afw,Kfw,Cfw (if needed)
            ...
            'steps_ahead_loss_weights': self.steps_ahead_loss_weights if hasattr(self, 'steps_ahead_loss_weights') else None, # Weight of each step ahead prediction in loss. If None, will give all steps ahead equal weight of 1.
        }
        soft_defaults.update(kw_args)
        logger.info(f'BRAID.finetune args: {soft_defaults}')
    ...
    def fit(self, Y, Z=None, U=None, nx=None, n1=None, 
            ...
            model2_Cz_Full = True, # Important: If True, will fit model2_Cz even when n2=0!
            has_UFT=True, # (default=True) whether U should be modeled as feedthrough or not in Cfw(everywhere/nowhere).
            has_UFT_reg= True, # (default=True) Whether regression models should include U as feedthrough. If has_UFT=False, it will automatically be made False.
            observable_U_in_Kfw =False, #  (default: False) Whether U should be treated as observable and applied in forward recursions (Kfw) in multiple-steps-ahead cases where U actually exists
            observable_U_in_Cfw = False, # (default: False) Whether U should be treated as observable and applied in forward feedthroughs (Cfw/model_C) in multiple-steps-ahead cases where U actually exists. If has_UFT/has_UFT_reg is False, Cfw/model_C also won't use FT in forward prediction.
        ...
        if time_first:
            raise(Exception('Time being the first dim is not supported for BRAID yet. Please transpose your data to have time as the 2nd dimension.'))
        ...
        if Y is not None:
            ...
            isTrialBased = isinstance(U, (list, tuple))
        if isTrialBased:
            # Trial based learning
            if Y is not None:
                segment_length = [YThis.shape[1] for YThis in Y]
            else:
                segment_length = [UThis.shape[1] for UThis in U]
            if np.max(segment_length) != np.min(segment_length):
                raise(Exception('Trials must be same length for trial-based learning (but they spanned {} to {} samples)'.format(np.min(segment_length), np.max(segment_length))))
            ...
            def convertToCat(data):
                ...
                return dataArray.transpose(1, 2, 0).reshape(dataArray.shape[1], dataArray.shape[0]*dataArray.shape[2], order='F' )
            Y = convertToCat(Y)
            Z = convertToCat(Z)
            U = convertToCat(U)
        ...
        if create_val_from_training:
            if isTrialBased: # Train/validation sets must be a multiple of trial length
                test_sample_1 = int(np.floor(Ndat/self.block_samples * (1-validation_set_ratio)))*self.block_samples 
            else:
                test_sample_1 = int(np.floor(Ndat * (1-validation_set_ratio)))
            if Y is not None:
                Y_validation = Y[:, test_sample_1:]
                Y = Y[:, 0:test_sample_1] # Remove the validation data from the training data
            if Z is not None:
                Z_validation = Z[:, test_sample_1:]
                Z = Z[:, 0:test_sample_1] # Remove the validation data from the training data
            if U is not None:
                U_validation = U[:, test_sample_1:]
                U = U[:, 0:test_sample_1] # Remove the validation data from the training data
            Ndat_for_batch_size = np.min( (Ndat, Y_validation.shape[1]) ) # Make sure batch size is not too large for validation data
        else:
            Ndat_for_batch_size = Ndat
            # if Y_validation is not None:
            #     Ndat_for_batch_size = np.min( (Ndat, Y_validation.shape[1]) )

        if batch_size is not None:
            self.batch_size = batch_size

        num_batch=int(np.ceil((Ndat_for_batch_size-1)/self.block_samples/self.batch_size))
        if num_batch < 2:
            num_batch = 1
        ...
        if use_existing_prep_models:
            ...
            remove_flat_dims = self.remove_flat_dims if hasattr(self, 'remove_flat_dims') else False
            zscore_inputs = self.zscore_inputs if hasattr(self, 'zscore_inputs') else False
            zscore_inputs = self.zscore_inputs if hasattr(self, 'zscore_inputs') else False
        else:
            ...
            if remove_flat_dims or zscore_inputs:
                YPrepMap = self.get_input_prep_map(Y, signal_name='Y', remove_flat_dims=remove_flat_dims, 
                    zscore_inputs=zscore_inputs and YType == 'cont', zscore_per_dim=zscore_per_dim)
                UPrepMap = self.get_input_prep_map(U, signal_name='U', remove_flat_dims=remove_flat_dims, 
                    zscore_inputs=zscore_inputs and UType == 'cont', zscore_per_dim=zscore_per_dim)
                ZPrepMap = self.get_input_prep_map(Z, signal_name='Z', remove_flat_dims=remove_flat_dims, 
                    zscore_inputs=zscore_inputs and ZType == 'cont', zscore_per_dim=zscore_per_dim)
                
        if YPrepMap is not None: 
            if Y is not None: Y = YPrepMap.apply(Y)
            if YTrue is not None: YTrue = YPrepMap.apply(YTrue.T).T
            if Y_validation is not None: Y_validation = YPrepMap.apply(Y_validation)
        if ZPrepMap is not None: 
            if Z is not None: Z = ZPrepMap.apply(Z)
            if ZTrue is not None: ZTrue = ZPrepMap.apply(ZTrue.T).T
            if Z_validation is not None: Z_validation = ZPrepMap.apply(Z_validation)
        if UPrepMap is not None: 
            if U is not None: U = UPrepMap.apply(U)
            if U_validation is not None: U_validation = UPrepMap.apply(U_validation)
        ...
        Ndat = Y.shape[1] if Y is not None else U.shape[1]
        ny = Y.shape[0] if Y is not None else 0
        ...
        if (n2>0 or not skip_Cy) and Y is None:
            raise(Exception('Y cannot be None when n2>0 or skip_Cy=False!'))
    
        isOkY = getIsOk(Y, self.missing_marker)
        isOkZ = getIsOk(Z, self.missing_marker)
        isOkU = getIsOk(U, self.missing_marker)
        
        if A1_args is None: A1_args = A_args
        if A2_args is None: A2_args = A_args
        if K1_args is None: K1_args = K_args
        if K2_args is None: K2_args = K_args
        if Cy1_args is None: Cy1_args = Cy_args
        if Cy2_args is None: Cy2_args = Cy_args
        if Cz1_args is None: Cz1_args = Cz_args
        if Cz2_args is None: Cz2_args = Cz_args

        A1_args, K1_args, Cy1_args, Cz1_args, A2_args, K2_args, Cy2_args, Cz2_args = \
        ...
        self.has_UFT= (has_UFT and self.nu>0)
        self.has_UFT_reg= (has_UFT_reg and self.nu>0)
        self.observable_U_in_Kfw = (observable_U_in_Kfw and self.nu>0 and self.enable_forward_pred)
        self.observable_U_in_Cfw = (observable_U_in_Cfw and self.nu>0 and self.enable_forward_pred)
        # if self.observable_U_in_Cfw and not self.has_UFT:
            # logger.warning('WARNING: user provided has_UFT=False, observable_U_in_Cfw=True. This case is not supported therefore will set observable_U_in_Cfw=False')
            # self.observable_U_in_Cfw = False
            # raise(Exception('has_UFT=False with observable_U_in_Kfw=True case is not supported!'))


        # Prepare the network
        model1, model1_Cy, model1_Cy_fw, model2, model2_Cz, model2_Cz_fw = self.build_models()
        if trainableParams is not None:
            MainModelSetTrainableParameters(
                model1=model1, model1_Cy=model1_Cy, model1_Cy_fw=model1_Cy_fw, 
                model2=model2, model2_Cz=model2_Cz, model2_Cz_fw=model2_Cz_fw, 
                **trainableParams)
        ...
        steps_ahead, steps_ahead_loss_weights, steps_ahead_model1, steps_ahead_loss_weights_model1, model1_orig_step_inds \
        ...
        get_steps_ahead_from_model1 = lambda L: [item for ind, item in enumerate(L) if ind in model1_orig_step_inds] if L is not None else None
        # np.testing.assert_equal(steps_ahead, get_steps_ahead_from_model1(steps_ahead_model1))

        # Whether need a separate Cfw/model_C is needed:
        need_Cfw = np.any(np.array(steps_ahead)!=1) and ((self.nu > 0 and has_UFT and not self.observable_U_in_Cfw) or self.has_Dyz)
        self.need_Cfw = need_Cfw
        need_fw_reg_models = np.any(np.array(steps_ahead)!=1) and ((self.nu > 0 and has_UFT_reg and not self.observable_U_in_Cfw) or self.has_Dyz)
        self.need_fw_reg_models = need_fw_reg_models
        ...
        # Stage 1
        if n1 > 0:
            if epochs > 0 or not skip_predictions or ((n2 > 0 or not skip_Cy) and epochs > 0):
                # Prepare input to RNN model1
                YU, FT_in, n1_in1 = self.prepare_inputs_to_model1(Y, U) #  Accounted for no feedthrough option. Supported U as n1_in for model1
                YU_validation, FT_in_validation, n1_in1_validation = self.prepare_inputs_to_model1(Y_validation, U_validation)
            if epochs > 0:
                # Learn RNN model1
                if verbose:
                    logger.info(f'Stage 1: Learning A11, K1, Cz1 (ZType: {ZType})')
                history1 = model1.fit(Y_in=YU, Y_out=Z, FT_in=FT_in, n1_in=n1_in1,
                                    Y_in_val=YU_validation, Y_out_val=Z_validation, FT_in_val=FT_in_validation, n1_in_val=n1_in1_validation, 
                                    **rnn_fit_args)
                logs['model1'] = convertHistoryToDict(history1)
            """
            ...
            if not skip_predictions or ((n2 > 0 or not skip_Cy) and epochs > 0) or verbose:
                preds1 = model1.predict(YU, FT_in=FT_in, n1_in=n1_in1) # Supported n1_in so that if observable_U_in_Kfw=True, then n1_in be the input U_steps.
                allXp1_steps = preds1[:len(steps_ahead_model1)] # this is  [X^{0|-1},X^{1|0},...], [X^{1|-1},X^{2|0},...]
                allZp1_steps = preds1[len(steps_ahead_model1):2*len(steps_ahead_model1)] # this is [Z^{0|-1},Z^{1|0},...], [Z^{1|-1},Z^{2|0},...]
                ...
                if YU_validation is not None:
                    ...
                    allZp1_steps_val = preds1_val[len(steps_ahead_model1):2*len(steps_ahead_model1)]
                else:
                    allXp1_steps_val, allZp1_steps_val = None, None

                # Prepare input to regression model1_Cy (regression from state to neural activity)
                allXp1U_steps, allXp1U_steps_Shifted, YRep1 = self.prepare_inputs_to_model1_Cy(Y, U, allXp1_steps, steps_ahead_model1)
                allXp1U_steps_val, allXp1U_steps_Shifted_val, YRep1_val = self.prepare_inputs_to_model1_Cy(Y_validation, U_validation, allXp1_steps_val, steps_ahead_model1)
                if need_fw_reg_models: # We have feedthrough and multi-step ahead, so we also need fw models
                    ...
                    allXp1_steps_Shifted = self.prepare_inputs_to_model1_Cy_fw(allXp1_steps, steps_ahead_model1)
                    allXp1_steps_Shifted_val = self.prepare_inputs_to_model1_Cy_fw(allXp1_steps_val, steps_ahead_model1)

            if (n2 > 0 or skip_Cy is False) and not model1_Cy_Full:
                # Learn model1_Cy (regression from state to neural activity)
                steps_ahead_model1_Cy, keeper_func_model1_Cy = self.keep_steps_and_cat_for_reg_model(get_steps_ahead_from_model1(steps_ahead_model1), return_keeper_func=True)
                if need_fw_reg_models:
                    steps_ahead_model1_Cy_fw, keeper_func_model1_Cy_fw = self.keep_steps_and_cat_for_reg_model(get_steps_ahead_from_model1(steps_ahead_model1), is_fw=True, return_keeper_func=True)
                if epochs > 0:
                    # Form neural regression training data considering required steps ahead
                    allXp1U_steps_shiftedCat = self.keep_steps_and_cat_for_reg_model(get_steps_ahead_from_model1(allXp1U_steps_Shifted))
                    YRep1Cat = self.keep_steps_and_cat_for_reg_model(get_steps_ahead_from_model1(YRep1))
                    allXp1U_val_steps_shiftedCat = self.keep_steps_and_cat_for_reg_model(get_steps_ahead_from_model1(allXp1U_steps_Shifted_val))
                    Y_validation_Rep1Cat = self.keep_steps_and_cat_for_reg_model(get_steps_ahead_from_model1(YRep1_val))

                    if verbose:
                        logger.info(f'Stage 1: Learning Cy1 (YType: {YType})')
                    history1_Cy = model1_Cy.fit(allXp1U_steps_shiftedCat, YRep1Cat, 
                                X_in_val=allXp1U_val_steps_shiftedCat, X_out_val=Y_validation_Rep1Cat, 
                                **reg_fit_args)
                    logs['model1_Cy'] = convertHistoryToDict(history1_Cy)

                    if need_fw_reg_models: # We have feedthrough and multi-step ahead, so we also need fw models (that wouldn't use the feedthrough)
                        # Form neural regression training data considering required steps ahead
                        allXp1_steps_shifted_fwCat = self.keep_steps_and_cat_for_reg_model(get_steps_ahead_from_model1(allXp1_steps_Shifted), is_fw=True)
                        YRep1_fwCat = self.keep_steps_and_cat_for_reg_model(get_steps_ahead_from_model1(YRep1), is_fw=True)
                        allXp1_val_steps_shifted_fwCat = self.keep_steps_and_cat_for_reg_model(get_steps_ahead_from_model1(allXp1_steps_Shifted_val), is_fw=True)
                        Y_validation_Rep1_fwCat = self.keep_steps_and_cat_for_reg_model(get_steps_ahead_from_model1(YRep1_val), is_fw=True)

                        if verbose:
                            logger.info(f'Stage 1: Learning Cy1_fw (YType: {YType})')
                        history1_Cy_fw = model1_Cy_fw.fit(allXp1_steps_shifted_fwCat, YRep1_fwCat, 
                                    X_in_val=allXp1_val_steps_shifted_fwCat, X_out_val=Y_validation_Rep1_fwCat, 
                                    **reg_fit_args)
                        logs['model1_Cy_fw'] = convertHistoryToDict(history1_Cy_fw)
        ...
        if (epochs > 0 or not skip_predictions) and (n2 > 0 or skip_Cy is False):
            ...
            Y_in_resU, allYp1_steps, n1_in = self.prepare_inputs_to_model2(model1_Cy, model1_Cy_fw,  Y, U, allXp1_steps, allXp1U_steps) 
            Y_in_res_valU, allYp1_steps_val, n1_in_val = self.prepare_inputs_to_model2(model1_Cy, model1_Cy_fw, Y_validation, U_validation, allXp1_steps_val, allXp1U_steps_val)
        ...
        if n2 > 0:
            if epochs > 0:
                ...
                if not self.observable_U_in_Cfw:
                    FT_in2 = U if self.has_UFT else None
                    ...
                    if self.has_UFT:
                        ...
                        FT_in2 = None
                        FT_in2_validation = None
                # prior_preds[0] = np.ones_like(prior_preds[0])*self.missing_marker ####Temp!!!
                history2 = model2.fit(Y_in=Y_in_resU, Y_out=Y, FT_in=FT_in2, n1_in=n1_in, prior_pred=prior_preds, \
                                    Y_in_val=Y_in_res_valU, Y_out_val=Y_validation, FT_in_val=FT_in2_validation, n1_in_val=n1_in_val, prior_pred_val=prior_preds_vals, \
                                    prior_pred_shift_by_one=True, \
                                    **rnn_fit_args) # Made feedthrough optional. Supported n1_in so that if nu>0 and observable_U_in_Kfw=True, then n1_in be X1 concatenated with the input.
                logs['model2'] = convertHistoryToDict(history2)
            if not skip_predictions or verbose:
                preds2 = model2.predict(Y_in_resU, FT_in=FT_in2, n1_in=n1_in, prior_pred=prior_preds, prior_pred_shift_by_one=True) 
        ...
        if not skip_predictions:
            allX_steps_val = None
            if n2 == 0:
                allX_steps = [allXp1_steps[model1_orig_step_inds[saInd]] for saInd in range(len(steps_ahead))]
                allX_steps_val = [allXp1_steps_val[model1_orig_step_inds[saInd]] for saInd in range(len(steps_ahead))] if allXp1_steps_val is not None else None
            elif n1 == 0:
            ...
            allX = np.empty((nx, Ndat)); allX[:] = np.nan

        if Z is not None and nz > 0 and (model2_Cz_Full or n1 == 0 or (n1 > 0 and n2 > 0 and allow_nonzero_Cz2)):
            if epochs > 0 or not skip_predictions:
                ...
                allXpForCz, priorForCzFit, ZRep2 = self.prepare_inputs_to_model2_Cz(Y, Z, U, allX_steps, allXp2_steps, get_steps_ahead_from_model1(allZp1_steps), steps_ahead)
                allXpForCz_val, priorForCzFit_val, ZRep2_val = self.prepare_inputs_to_model2_Cz(Y_validation, Z_validation, U_validation, allX_steps_val, allXp2_steps_val, get_steps_ahead_from_model1(allZp1_steps_val), steps_ahead)
                
                steps_ahead_model2_Cz, keeper_func_model2_Cz = self.keep_steps_and_cat_for_reg_model(steps_ahead, return_keeper_func=True)
                ...
                allXpForCz_valCat = self.keep_steps_and_cat_for_reg_model(allXpForCz_val)
                priorForCzFit_valCat = self.keep_steps_and_cat_for_reg_model(priorForCzFit_val)
                ...
                if need_fw_reg_models: # We have feedthrough and multi-step ahead, so we also need fw models
                    ...
                    allXpForCz_fw, priorForCzFit_fw = self.prepare_inputs_to_model2_Cz_fw(allX_steps, allXp2_steps, get_steps_ahead_from_model1(allZp1_steps), steps_ahead)
                    allXpForCz_fw_val, priorForCzFit_fw_val = self.prepare_inputs_to_model2_Cz_fw(allX_steps_val, allXp2_steps_val, get_steps_ahead_from_model1(allZp1_steps_val), steps_ahead) if Y_validation is not None else None
                    # Concatenate the relevant time steps
                    steps_ahead_model2_Cz_fw, keeper_func_model2_Cz_fw = self.keep_steps_and_cat_for_reg_model(steps_ahead, is_fw=True, return_keeper_func=True)
                    allXpForCz_fwCat = self.keep_steps_and_cat_for_reg_model(allXpForCz_fw, is_fw=True)
                    priorForCzFit_fwCat = self.keep_steps_and_cat_for_reg_model(priorForCzFit_fw, is_fw=True)
                    ZRep_fwCat = self.keep_steps_and_cat_for_reg_model(ZRep2, is_fw=True)
                    allXpForCz_fw_valCat = self.keep_steps_and_cat_for_reg_model(allXpForCz_fw_val, is_fw=True)
                    priorForCzFit_fw_valCat = self.keep_steps_and_cat_for_reg_model(priorForCzFit_fw_val, is_fw=True)
                    ZRep2_fw_valCat = self.keep_steps_and_cat_for_reg_model(ZRep2_val, is_fw=True)
            ...
            if epochs > 0:
                ...
                history2_Cz = model2_Cz.fit(allXpForCzCat, ZRepCat, prior_pred=priorForCzFitCat, \
                                X_in_val=allXpForCz_valCat, X_out_val=ZRep2_valCat, prior_pred_val=priorForCzFit_valCat, \
                                **reg_fit_args)
                logs['model2_Cz'] = convertHistoryToDict(history2_Cz)

                if need_fw_reg_models: # We have feedthrough and multi-step ahead, so we also need fw models
                    if verbose:
                        logger.info(f'Stage 2: Learning Cz2_fw (ZType: {ZType})')
                    history2_Cz_fw = model2_Cz_fw.fit(allXpForCz_fwCat, ZRep_fwCat, prior_pred=priorForCzFit_fwCat, \
                                    X_in_val=allXpForCz_fw_valCat, X_out_val=ZRep2_fw_valCat, prior_pred_val=priorForCzFit_fw_valCat, \
                                    **reg_fit_args)
                    logs['model2_Cz_fw'] = convertHistoryToDict(history2_Cz_fw)
        ...
        if model1_Cy_Full:
            if epochs > 0 or not skip_predictions:
                ...
                allXpUForCy_steps, allXpUForCy_steps_Shifted, YRep2 = self.prepare_inputs_to_model1_Cy(Y, U, allX_steps, steps_ahead)
                allXpUForCy_steps_val, allXpUForCy_steps_Shifted_val, YRep2_val = self.prepare_inputs_to_model1_Cy(Y_validation, U_validation, allX_steps_val, steps_ahead)
                
                steps_ahead_model1_Cy = self.keep_steps_and_cat_for_reg_model(steps_ahead)
                allXpUForCyCat = self.keep_steps_and_cat_for_reg_model(allXpUForCy_steps_Shifted)
                YRep2Cat = self.keep_steps_and_cat_for_reg_model(YRep2)
                allXpUForCy_valCat = self.keep_steps_and_cat_for_reg_model(allXpUForCy_steps_Shifted_val)
                ...
                if need_fw_reg_models: # We have feedthrough and multi-step ahead, so we also need fw models
                    ...
                    allXpForCy_steps_Shifted = self.prepare_inputs_to_model1_Cy_fw(allX_steps, steps_ahead)
                    allXpForCy_steps_Shifted_val = self.prepare_inputs_to_model1_Cy_fw(allX_steps_val, steps_ahead)

                    steps_ahead_model1_Cy_fw = self.keep_steps_and_cat_for_reg_model(steps_ahead, is_fw=True)
                    allXpForCy_fwCat = self.keep_steps_and_cat_for_reg_model(allXpForCy_steps_Shifted, is_fw=True)
                    YRep2_fwCat = self.keep_steps_and_cat_for_reg_model(YRep2, is_fw=True)
                    allXpForCy_fw_valCat = self.keep_steps_and_cat_for_reg_model(allXpForCy_steps_Shifted_val, is_fw=True)
                    Y_validation_Rep2_fwCat = self.keep_steps_and_cat_for_reg_model(YRep2_val, is_fw=True)

            # Learn Cy (regression from all states to neural activity)
            if epochs > 0:
                if verbose:
                    logger.info(f'Learning full Cy (YType: {YType})')
                history1_Cy = model1_Cy.fit(allXpUForCyCat, YRep2Cat, 
                                X_in_val=allXpUForCy_valCat, X_out_val=Y_validation_Rep2Cat, 
                                **reg_fit_args)
                logs['model1_Cy'] = convertHistoryToDict(history1_Cy)
                ...
                if need_fw_reg_models: # We have feedthrough and multi-step ahead, so we also need fw models
                    ...
                    history1_Cy_fw = model1_Cy_fw.fit(allXpForCy_fwCat, YRep2_fwCat, 
                                    X_in_val=allXpForCy_fw_valCat, X_out_val=Y_validation_Rep2_fwCat, 
                                    **reg_fit_args)
                    logs['model1_Cy_fw'] = convertHistoryToDict(history1_Cy_fw)
    ...
    def hasBlownUp(self):
        blown_up = False
        if hasattr(self, 'model1') and  hasattr(self.model1, 'blown_up') and self.model1.blown_up:
            blown_up = True
        if hasattr(self, 'model1_Cy') and  hasattr(self.model1_Cy, 'blown_up') and self.model1_Cy.blown_up:
            blown_up = True
        if hasattr(self, 'model1_Cy_fw') and  hasattr(self.model1_Cy_fw, 'blown_up') and self.model1_Cy_fw.blown_up:
            blown_up = True
        if hasattr(self, 'model2') and  hasattr(self.model2, 'blown_up') and self.model2.blown_up:
            blown_up = True
        if hasattr(self, 'model2_Cz') and  hasattr(self.model2_Cz, 'blown_up') and self.model2_Cz.blown_up:
            blown_up = True
        if hasattr(self, 'model2_Cz_fw') and  hasattr(self.model2_Cz_fw, 'blown_up') and self.model2_Cz_fw.blown_up:
            blown_up = True
        return blown_up


    def keep_steps_and_cat_for_reg_model(self, input_list_shifted, is_fw=False, return_keeper_func=False):
        steps_ahead, steps_ahead_loss_weights, steps_ahead_model1, steps_ahead_loss_weights_model1, model1_orig_step_inds \
             = self.get_model_steps_ahead(self.steps_ahead, self.steps_ahead_loss_weights)

        if is_fw: # Preparing input for fw models
            # Is intended for _fw regression models, so only keeping weighted steps_ahead that are more than 1
            weighted_steps_ahead_inds = [saInd for saInd in range(len(steps_ahead)) if (steps_ahead_loss_weights is None or steps_ahead_loss_weights[saInd]!=0) and steps_ahead[saInd] != 1]
        else: # Preparing input for non-fw models
            if not self.need_fw_reg_models: # we don't have separate fw models (e.g. not hasFT), so we should keep all steps ahead for non-fw regression models
                weighted_steps_ahead_inds = [saInd for saInd in range(len(steps_ahead)) if (steps_ahead_loss_weights is None or steps_ahead_loss_weights[saInd]!=0)]
            else: # we have separate fw models and will only keep 1-step ahead for non-fw regression models
                weighted_steps_ahead_inds = [saInd for saInd in range(len(steps_ahead)) if (steps_ahead_loss_weights is None or steps_ahead_loss_weights[saInd]!=0) and steps_ahead[saInd] == 1]

        if steps_ahead_loss_weights is not None and len(np.unique(
                [w for ind, w in enumerate(steps_ahead_loss_weights) if ind in weighted_steps_ahead_inds]
            )) > 1:
            raise(Exception('Fitting model1_Cy and model2_Cz with different step ahead weights is not supported yet!'))
        ...
        if input_list_shifted is not None:
            ...
            if isinstance(kept_items[0], (np.ndarray)):
                non_missing_inds_at_start = [sav-1 for sav in keep_w_items(steps_ahead)] # This many samples of predictions will be nan's and should be removed
                kept_items_ok = [kept_items[i][:, non_missing_inds_at_start[i]:] for i in range(len(kept_items))]
    ...
    def prepare_inputs_to_model1_Cy(self, Y, U, allXp1_steps, steps_ahead): # Made feedthrough optional. Fixed the issue in preparing allXp1U_steps and allXp1U_steps_Shifted
        ...
        if self.nu > 0 and self.has_UFT_reg:
            # TO DO: check the following, seems wrong that U will be shifted twice to get allXp1U_steps_Shifted
            # UShifted_steps = shift_ms_to_1s_series(U, steps_ahead, self.missing_marker, time_first=False) 
            # UReverseShifted_steps = shift_1s_to_ms_series(UShifted_steps, steps_ahead, self.missing_marker, time_first=False)
            UReverseShifted_steps = shift_1s_to_ms_series(U, steps_ahead, self.missing_marker, time_first=False)
            ...
            for allXp1_this_step, UReverseShifted_this_step in zip(allXp1_steps, UReverseShifted_steps):
                allXp1U_this_step = np.concatenate( (allXp1_this_step, UReverseShifted_this_step) )
                allXp1U_steps.append(allXp1U_this_step)
        else:
    ...
    def prepare_inputs_to_model2_Cz_fw(self, allX_steps, allXp2_steps, allZp_steps, steps_ahead):
        """Returns shifted inputs, shifted priors, and repeated outputs required to train model2_Cz
    ...
    def prepare_inputs_to_model1(self, Y, U): # Supported preparing n1_in. If observable_U_in_Kfw: n1_in = U_steps.
        ...
        if Y is None and U is None:
            return None, None, None
        if self.nu > 0 and self.ny > 0:
            YU = np.concatenate([Y, U], axis=0)
        elif self.ny > 0:
            YU = Y
        else:
            YU = U
        if self.has_UFT and self.has_Dyz:
            FT_in = YU
        elif self.has_UFT:
            FT_in = U
        elif self.has_Dyz:
        ...
        if U is not None and self.nu>0 and self.observable_U_in_Kfw:
            ...
            n1_in = None
        return YU, FT_in, n1_in

    def prepare_inputs_to_model2(self, model1_Cy, model1_Cy_fw, Y, U, allXp1_steps, allXp1U_steps): # Supported returning n1_in=allXp1U_steps if observable_U_in_Kfw=True
        ...
        if Y is None:
            return None, None, None
        if self.n1 > 0:
            allXp = allXp1_steps[0] # 1-step
            Y_in_res = np.concatenate( (allXp, Y), axis=0 )
        else:
            Y_in_res = Y
        if self.nu > 0:
            Y_in_resU = np.concatenate([Y_in_res, U], axis=0)
        else:
            Y_in_resU = Y_in_res

        # Compute forward pred inputs for model 2
        if self.n1 > 0 and (U is not None and self.nu>0 and self.observable_U_in_Kfw):
            if self.has_UFT_reg:
                n1_in = allXp1U_steps
            else:
        ...
        if self.n1 > 0: # For step > 1 model1_Cy_fw should be applied to allXp1_steps. Similar thing is done in DPAModel.predict
            _, _, steps_ahead_model1, _, model1_orig_step_inds = self.get_model_steps_ahead(self.steps_ahead)
            allYp_steps = []
            for si, step in enumerate(steps_ahead_model1):
                if not self.need_fw_reg_models or step==0 or step==1:
                    allYp_This_steps = model1_Cy.predict(allXp1U_steps[si]) # allXp1U is the actual input to model1_Cy (may not include U)
                else:
                    allYp_This_steps = model1_Cy_fw.predict(allXp1_steps[si])
                allYp_steps.append(allYp_This_steps)
        else:
            allYp_steps = None
    ...
    def build_models(self):
        ...
        if self.n1 > 0:
            if self.has_UFT:
                ...
                nft = self.ny if self.has_Dyz else 0
            # nft = self.ny+self.nu if self.has_Dyz else self.nu
            ...
            model1 = RNNModel(self.n1, self.ny+self.nu, 
                        ...
                        steps_ahead=steps_ahead_model1, steps_ahead_loss_weights=steps_ahead_loss_weights_model1,
                        observable_U_in_Cfw=self.observable_U_in_Cfw, #Added setting to use feedthrough in Cfw
                        name='RNN1_', 
                        log_dir=this_log_dir, 
                        missing_marker=self.missing_marker, 
                        cell_args=rnn_cell_args) # To predict z from past y
        else:
            model1 = None
        ...
        if self.n2 > 0:
            ...
            model2 = RNNModel(self.n2, self.n1+self.ny+self.nu, 
                        ...
                        log_dir=this_log_dir, 
                        missing_marker=self.missing_marker, 
                        cell_args=rnn_cell_args) # To predict y from past y
        else:
            model2 = None

        if self.nz > 0 and (self.model2_Cz_Full or \
                            self.n1 == 0 or \
                           (self.n1 > 0 and self.n2 > 0 and self.allow_nonzero_Cz2)
            ):
            ...
            if self.has_UFT_reg:
                input_dim += self.nu
            model2_Cz = RegressionModel(input_dim,
                        self.nz, 
                        name='model2_Cz_',
                        log_dir=this_log_dir, 
                        missing_marker=self.missing_marker, 
        ...
        return model1, model1_Cy, model1_Cy_fw, model2, model2_Cz, model2_Cz_fw

    def print_summary(self):
        out = ''
        out = f'BRAID model summary (nx={self.nx}, ny={self.ny}, nz={self.nz}, nu={self.nu}, steps ahead: {self.steps_ahead}):'
        if self.model1 is not None:
            out += '\n- RNN model1: {}D state, {}D in, {}D out, {}D feedthrough, has prior: {}'.format(self.model1.nx, self.model1.ny, self.model1.ny_out, self.model1.nft, self.model1.has_prior_pred)
        if self.model2 is not None:
            out += '\n- RNN model2: {}D state, {}D in ({}D x1 + {}D y), {}D out, {}D feedthrough, has prior: {}'.format(self.model2.nx, self.model2.ny, self.n1, self.ny, self.model2.ny_out, self.model2.nft, self.model2.has_prior_pred)
        if self.model1_Cy is not None:
            out += '\n- Reg model1_Cy: {}D in, {}D out, full refit: {}'.format(self.model1_Cy.n_in, self.model1_Cy.n_out, self.model1_Cy_Full)
        if self.model1_Cy_fw is not None:
            out += '\n- Reg model1_Cy_fw: {}D in, {}D out, full refit: {}'.format(self.model1_Cy_fw.n_in, self.model1_Cy_fw.n_out, self.model1_Cy_Full)
        if self.model2_Cz is not None:
            out += '\n- Reg model2_Cz: {}D in, {}D out, full refit: {}'.format(self.model2_Cz.n_in, self.model2_Cz.n_out, self.model2_Cz_Full)
        if self.model2_Cz_fw is not None:
            out += '\n- Reg model2_Cz_fw: {}D in, {}D out, full refit: {}'.format(self.model2_Cz_fw.n_in, self.model2_Cz_fw.n_out, self.model2_Cz_Full)
        print(out)
        return out

    def plot_log_traces(self, ax, logs=None, label='', keys=None, linestyles=None, colors=None):
        if logs is None:
            logs = self.logs
        if keys is None:
            keys = ['loss', 'val_loss']
        ...
        for mi, (model_name, log) in enumerate(logs.items()):
            ...
            for ki, key in enumerate(keys):
                if key in log['history']:
                    ax.plot(log['epoch'], np.log10(log['history'][key]), c=color, label=f'{labelThis}{key}', linestyle=lineStyles[ki%len(lineStyles)])
                    if 'best_epoch' in log['params']:
                    ...
                    if pick_ind is not None:
                        ax.scatter(log['epoch'][pick_ind], np.log10(log['history'][key][pick_ind]), c=color, marker='x')    
    def plot_logs(self, logs=None, keys=None, titleStr='', skip_existing=False, saveFile=None, saveExtensions=None, fig=None, figsize=None):
    ...
    def set_steps_ahead(self, steps_ahead, update_rnn_model_steps=True): 
        steps_ahead, _, steps_ahead_model1, _, _ \
    ...
    def discardModels(self):
        ...
        model_names = ['model1', 'model2', 'model1_Cy', 'model1_Cy_fw', 'model2_Cz', 'model2_Cz_fw']
    ...
    def restoreModels(self):
        ...
        if not hasattr(self, 'has_Dyz'): self.has_Dyz = False
        if not hasattr(self, 'A1_args'): self.A1_args = self.A_args
        if not hasattr(self, 'A2_args'): self.A2_args = self.A_args
        if not hasattr(self, 'K1_args'): self.K1_args = self.K_args
        if not hasattr(self, 'K2_args'): self.K2_args = self.K_args
        if not hasattr(self, 'Cy1_args'): self.Cy1_args = self.Cy_args
        if not hasattr(self, 'Cy2_args'): self.Cy2_args = self.Cy_args
        if not hasattr(self, 'Cz1_args'): self.Cz1_args = self.Cz_args
        if not hasattr(self, 'Cz2_args'): self.Cz2_args = self.Cz_args
        if not hasattr(self, 'model1_Cy_fw'): self.model1_Cy_fw = None
        if not hasattr(self, 'model2_Cz_fw'): self.model2_Cz_fw = None

        # End
        _, _, yDist = self.prep_observation_for_training(None, self.YType)
        _, _, zDist = self.prep_observation_for_training(None, self.ZType)
        if self.n1 > 0 and not isinstance(self.model1, (RNNModel)):
            if isinstance(self.model1, ReconstructionInfo) and hasattr(self.model1, 'constructor_kwargs'):
                if 'smooth_input' in self.model1.constructor_kwargs and not self.model1.constructor_kwargs['smooth_input']: # For backward compatibility
                    self.model1.constructor_kwargs.pop('smooth_input')
                    self.model1.constructor_kwargs.pop('smoother_args')
                self.model1 = (RNNModel(**self.model1.constructor_kwargs)).reconstruct(self.model1)
            else:
                w = self.model1
                ...
                self.model1 = RNNModel(self.n1, self.ny+self.nu, self.block_samples, self.batch_size, ny_out=self.nz, nft=nft, 
                                                ...
                                                out_dist=zDist, 
                                                missing_marker=self.missing_marker, cell_args=rnn_cell_args)
                w_rand = self.model1.model.get_weights()
                if len(w_rand) == len(w)+1 and np.allclose( w_rand[-1], np.zeros_like(w_rand[-1]) ): # Backward compatibility
                    w += [w_rand[-1]]
                self.model1.model.set_weights(w)
            self.model1.set_batch_size(1)
            
        if self.model1_Cy is not None and not isinstance(self.model1_Cy, (RegressionModel)):
            if isinstance(self.model1_Cy, ReconstructionInfo) and hasattr(self.model1_Cy, 'constructor_kwargs'):
                model1_Cy_BU = self.model1_Cy
                w = model1_Cy_BU.weights
                self.model1_Cy = (RegressionModel(**self.model1_Cy.constructor_kwargs)).reconstruct(self.model1_Cy)
            else:
                w = self.model1_Cy
                ...
                reg_args = copy.deepcopy(self.Cy1_args)
                self.model1_Cy = RegressionModel(CyInputDim, self.ny, missing_marker=self.missing_marker, **reg_args)
                self.model1_Cy.model.set_weights(w)

        if self.model1_Cy_fw is not None and not isinstance(self.model1_Cy_fw, (RegressionModel)):
            self.model1_Cy_fw = (RegressionModel(**self.model1_Cy_fw.constructor_kwargs)).reconstruct(self.model1_Cy_fw)

        if self.n2 > 0 and not isinstance(self.model2, (RNNModel)):
            if isinstance(self.model2, ReconstructionInfo) and hasattr(self.model2, 'constructor_kwargs'):
                if 'smooth_input' in self.model2.constructor_kwargs and not self.model2.constructor_kwargs['smooth_input']: # For backward compatibility
                    self.model2.constructor_kwargs.pop('smooth_input')
                    self.model2.constructor_kwargs.pop('smoother_args')
                self.model2 = (RNNModel(**self.model2.constructor_kwargs)).reconstruct(self.model2)
            else:
                w = self.model2
                rnn_cell_args = copy.deepcopy({'ASettings': self.A2_args, 'KSettings': self.K2_args, 'CSettings': self.Cy2_args})
                self.model2 = RNNModel(self.n2, self.ny+self.n1+self.nu, self.block_samples, self.batch_size, ny_out=self.ny, nft=self.nu, 
                                                LSTM_cell=hasattr(self, 'LSTM_cell') and self.LSTM_cell, 
                                                has_prior_pred=True, # From stage 1
                                                out_dist=yDist, 
                                                missing_marker=self.missing_marker, cell_args=rnn_cell_args)
        ...
        if self.nz > 0 and self.model2_Cz is not None and not isinstance(self.model2_Cz, (RegressionModel)):
            if isinstance(self.model2_Cz, ReconstructionInfo) and hasattr(self.model2_Cz, 'constructor_kwargs'):
                self.model2_Cz = (RegressionModel(**self.model2_Cz.constructor_kwargs)).reconstruct(self.model2_Cz)
            else:
                w = self.model2_Cz
                if not self.model2_Cz_Full:
                ...
                reg_args = copy.deepcopy(self.Cz2_args)
                reg_args['has_prior_pred'] = True # From stage 1
                self.model2_Cz = RegressionModel(CzInputDim, self.nz, missing_marker=self.missing_marker, **reg_args)
                self.model2_Cz.model.set_weights(w)

        if self.nz > 0 and self.model2_Cz_fw is not None and not isinstance(self.model2_Cz_fw, (RegressionModel)):
            self.model2_Cz_fw = (RegressionModel(**self.model2_Cz_fw.constructor_kwargs)).reconstruct(self.model2_Cz_fw)
    ...
    def saveToFile(self, filePath, saveTFModels=False):
        ...
        mh.make_sure_parent_dir_exists(filePath)
    ...
    def getLSSM(self):
        ...
        if self.n1 > 0:
            ...
            if 'unifiedAK' in self.model1.cell_args and self.model1.cell_args['unifiedAK']:
            ...
            if self.nu > 0:
                ...
                K = K[:, :self.ny]      # n1 x ny
            if hasattr(self.model1.rnn.cell, 'Afw') and self.steps_ahead is not None and (len(self.steps_ahead) > 1 or self.steps_ahead[0] != 1):
                if 'unifiedAK' in self.model1.cell_args and self.model1.cell_args['unifiedAK']:
                    Afw = self.model1.rnn.cell.Afw.get_weights()[0][:self.n1,:].T
                else:
                    Afw = self.model1.rnn.cell.Afw.get_weights()[0].T

            if self.nz > 0 and self.model2_Cz is not None and self.n2 == 0 and self.model2_Cz_Full: # Use the regression for CzDz instead of the one from RNN
            ...
            if hasattr(self, 'has_Dyz') and self.has_Dyz:
                ...
                CzDz = np.concatenate( (CzDz[:, :self.n1], CzDz[:, (self.n1+self.ny):]), axis=1 )

        if self.model1_Cy is not None or self.model1_Cy_Full:
            CyDy = self.model1_Cy.model.get_weights()[0].T  # ny x (n1+(nu)) or ny x (n1+n2+(nu))
        ...
        if self.n2 > 0:
            ...
            if 'unifiedAK' in self.model2.cell_args and self.model2.cell_args['unifiedAK']:
            ...
            if hasattr(self.model2.rnn.cell, 'Afw') and self.steps_ahead is not None and (len(self.steps_ahead) > 1 or self.steps_ahead[0] != 1):
                if 'unifiedAK' in self.model2.cell_args and self.model2.cell_args['unifiedAK']:
                    A2fw = self.model2.rnn.cell.Afw.get_weights()[0][:self.n2,:].T # n2 x n2
                    if self.model2.rnn.cell.n1_in > 0:
                        K2fw = self.model2.rnn.cell.Afw.get_weights()[0][self.n2:,:].T # n2 x (n1+nu)
                    else:
                        K2fw = np.zeros((self.n2,0))
                else:
                    A2fw = self.model2.rnn.cell.Afw.get_weights()[0].T # n2 x n2
                    if self.model2.rnn.cell.n1_in > 0:
                        K2fw = self.model2.rnn.cell.Kfw.get_weights()[0].T # n2 x (n1+nu)
                    else:
                        K2fw = np.zeros((self.n2,0))
                K2fw = K2fw[:, :self.n1] # n2 x n1
            ...
            if not self.model1_Cy_Full:
                if self.model1_Cy is None:
                    ...
                    Cy1 = CyDy[:, :self.n1]
                    Cy2 = CyDy2[:, :self.n2]
                    Dy = CyDy[:, self.n1:] + CyDy2[:, self.n2:] # Add up any feedthrough from two stages
            ...
            if self.nz > 0 and self.model2_Cz is not None:
                CzDz2 = self.model2_Cz.model.get_weights()[0].T # nz x (n2+(nu)) or nz x (n1+n2+(nu))
                # CzDz2_bias = self.model2_Cz.model.get_weights()[1]

                if hasattr(self, 'has_Dyz') and self.has_Dyz and (self.n1 == 0 or self.model2_Cz_Full):
                    Cz2Size = self.n2 if not self.model2_Cz_Full else self.n1+self.n2
                    Dyz = CzDz2[:, Cz2Size:(Cz2Size+self.ny)]
                    CzDz2 = np.concatenate( (CzDz2[:, :Cz2Size], CzDz2[:, (Cz2Size+self.ny):]), axis=1 )
            else:
                if not self.model2_Cz_Full:
            ...
            if (self.model2_Cz is None and 
                    (not hasattr(self,'allow_nonzero_Cz2') or \
                ...
                Cz1 = CzDz[:, :self.n1]
                Cz2 = CzDz2[:, :self.n2]
                Dz = CzDz[:, self.n1:] + CzDz2[:, self.n2:] # Add up any feedthrough from two stages
            ...
            if self.n1 == 0:
                ...
                if hasattr(self.model2.rnn.cell, 'Afw') and self.steps_ahead is not None and (len(self.steps_ahead) > 1 or self.steps_ahead[0] != 1):
                    A21fw = K2fw # n2 x n1
                    A12fw = np.zeros_like(A_KCy21).T
                    Afw = np.concatenate((
                        np.concatenate(( Afw   , A12fw ), axis=1),
                        np.concatenate(( A21fw , A2fw  ), axis=1)
                    ), axis=0)
        ...
        if self.model1_Cy_fw is not None:
            logger.warning('model1_Cy_fw exists in the model but will be ignored in getLSSM, since it is meant to be a subset of model1_Cy')
        if self.model2_Cz_fw is not None:
            logger.warning('model2_Cz_fw exists in the model but will be ignored in getLSSM, since it is meant to be a subset of model2_Cz')
        ...
        if Dz.size < 1: 
            Dz = np.zeros((self.nz,self.nu))
        A = A_KCy + np.matmul(K, Cy) # To do: Or, this could be Afw instead of A_KC + K@C
        B_KDy = np.concatenate((B_KDy1, B_KDy2), axis=0)
        B = B_KDy + np.matmul(K, Dy) # To do: Or, this could be constructed using Kfw instead of B_KD + K@D
        ...
        if Afw is not None:
            ...
            s.Afw = copy.deepcopy(s.A)
        setattr(s, 'info', {})
        s.info['logs'] = self.logs if hasattr(self, 'logs') else {}
        return s

    def setToLSSM(self, sys, model1_Cy_Full=True, model2_Cz_Full=True, allow_nonzero_Cz2=True, A_KC_is_block_diagonal=False, props={},
            YType=None, ZType=None,
            A_args = {}, K_args = {}, Cy_args = {}, Cz_args = {},    # Both stage 1 and 2 params
            A1_args = None, K1_args = None, Cy1_args = None, Cz1_args = None, # Stage 1 params
            A2_args = None, K2_args = None, Cy2_args = None, Cz2_args = None, # Stage 2 params
            ...
            has_UFT=True, # (default=True) whether U should be modeled as feedthrough or not in Cfw(everywhere/nowhere).
            has_UFT_reg=True, # Whether regression models should include U as feedthrough. If has_UFT=False, it will automatically be made False.
            observable_U_in_Kfw=False, # Whether U should be treated as observable and applied in forward recursions (Kfw) in multiple-steps-ahead cases where U actually exists
            observable_U_in_Cfw=False, # Whether U should be treated as observable and applied in forward recursions (Cfw) in multiple-steps-ahead cases where U actually exists
        ):
        """Sets the MainModel to be equal to an LSSM.
        ...
        self.A1_args, self.K1_args, self.Cy1_args, self.Cz1_args, self.A2_args, self.K2_args, self.Cy2_args, self.Cz2_args = \
            self.add_default_param_args(
                A1_args, K1_args, Cy1_args, Cz1_args, 
                A2_args, K2_args, Cy2_args, Cz2_args, 
                yDist, zDist
        ...
        if steps_ahead is not None and (len(steps_ahead) > 1 or steps_ahead[0] != 1) and not enable_forward_pred:
            raise(Exception('enable_forward_pred must be True if steps_ahead includes numbers other than 1'))
        ...
        self.has_UFT = (has_UFT and self.nu>0)
        self.has_UFT_reg = (has_UFT_reg and self.nu>0)
        self.observable_U_in_Kfw = (observable_U_in_Kfw and self.nu>0 and enable_forward_pred)
        self.observable_U_in_Cfw = (observable_U_in_Cfw and self.nu>0 and enable_forward_pred)

        steps_ahead, _, steps_ahead_model1, _, _ \
             = self.get_model_steps_ahead(steps_ahead)

        self.steps_ahead = steps_ahead
        need_Cfw = np.any(np.array(steps_ahead)!=1) and ((self.nu > 0 and self.has_UFT and not self.observable_U_in_Cfw) or self.has_Dyz)
        need_fw_reg_models = np.any(np.array(steps_ahead)!=1) and ((self.nu > 0 and self.has_UFT_reg and not self.observable_U_in_Cfw) or self.has_Dyz)

        A_KC_topRight = s.A_KC[:self.n1, self.n1:]
        topRightIsZero = np.linalg.norm(A_KC_topRight)/np.linalg.norm(s.A_KC) < 1e-8
        if self.n1 > 0 and self.n2 > 0 and not A_KC_is_block_diagonal and not topRightIsZero:
        ...
        if self.n1 > 0 and self.n2 > 0:
            ...
            if np.linalg.norm(A_KC_topRight)/np.linalg.norm(A_KC) > 1e-6:
                err = 'Top right {}x{} block of A_KC must be almost zero, otherwise BRAID model with n1={} will NOT be equivalent to the original LSSM.'.format(self.n1, self.n2, self.n1)
            ...
            if enable_forward_pred:
                ...
                if np.linalg.norm(A_topRight)/np.linalg.norm(A) > 1e-6:
                    err = 'Top right {}x{} block of A must be almost zero to enable forward prediction, otherwise BRAID model with n1={} will NOT be equivalent to the original LSSM in terms of forward prediction.'.format(self.n1, self.n2, self.n1)
        ...
        # Cz model
        self.model2_Cz_Full = model2_Cz_Full
        if self.model2_Cz_Full or self.n1 == 0: # Dz needs to be taken into account in model2_Cz
            CzThis = Cz
            DzThis = Dz 
            ...
            DzRNN1 = Dz
        if self.has_Dyz and (self.n1 == 0 or self.model2_Cz_Full):   # Dyz will be incorporated in model2_Cz
        ...
        if not allow_nonzero_Cz2 and not model2_Cz_Full and self.n1 > 0 and self.n2 > 0:
            if np.linalg.norm(CzDz)/np.linalg.norm(np.concatenate((Cz,Dz),axis=1)) < 1e-6:
                CzDz = np.zeros((CzDz.shape[0],0))
            elif CzDz.size > 0:
                raise(Exception('CzDz must be all zeros, otherwise BRAID model with n1={} will NOT be equivalent to the original LSSM.'.format(self.n1)))
        reg_args = copy.deepcopy(self.Cz2_args)
        reg_args['has_prior_pred'] = True # From stage 1
        if CzDz.size > 0:
            self.model2_Cz = RegressionModel(CzDz.shape[-1], self.nz, missing_marker=self.missing_marker, **reg_args)
            self.model2_Cz.model.set_weights( [CzDz.T] )
        else:
            self.model2_Cz = None

        if need_fw_reg_models and CzThis.size > 0:
            self.model2_Cz_fw = RegressionModel(CzThis.shape[-1], self.nz, missing_marker=self.missing_marker, **reg_args)            
        ...
        if self.n1 > 0:
            ...
            if self.has_Dyz and self.has_UFT:   # Dyz will be incorporated in RNN1
            ...
            if steps_ahead is not None and (len(steps_ahead) > 1 or steps_ahead[0] != 1):
                w['Afw'] = A[:self.n1, :self.n1].T
                if nft>0 and need_fw_reg_models:
                    w['Cfw'] = Cz[:, :self.n1].T
                if self.nu>0 and self.observable_U_in_Kfw:
                    w['Kfw'] = B[:self.n1, :].T
            n1_in1 = self.nu if self.nu > 0 and self.observable_U_in_Kfw else 0
            self.model1 = RNNModel(self.n1, self.ny+self.nu, self.block_samples, self.batch_size, ny_out=self.nz, nft=nft, n1_in=n1_in1,
                                            enable_forward_pred=enable_forward_pred,
                                            steps_ahead=steps_ahead_model1, 
                                            observable_U_in_Cfw=self.observable_U_in_Cfw,
                                            missing_marker=self.missing_marker, cell_args=rnn_cell_args, stateful=self.stateful)
            # self.model1.model.set_weights(w)
        ...
        # Cy model
        self.model1_Cy_Full = model1_Cy_Full
        if self.model1_Cy_Full or self.n2 == 0: # Dy will be taken into account in model1_Cy
            CyThis = Cy
            DyThis = Dy 
        ...
        reg_args = copy.deepcopy(self.Cy1_args)
        if CyDy.size > 0:
            self.model1_Cy = RegressionModel(CyDy.shape[-1], self.ny, missing_marker=self.missing_marker, **reg_args)            
            self.model1_Cy.model.set_weights( [CyDy.T] )
        else:
            self.model1_Cy = None

        if need_fw_reg_models and CyThis.size > 0:
            self.model1_Cy_fw = RegressionModel(CyThis.shape[-1], self.ny, missing_marker=self.missing_marker, **reg_args)            
            self.model1_Cy_fw.model.set_weights( [CyThis.T] )
        else:
            self.model1_Cy_fw = None

        # RNN2
        if self.n2 > 0:
            A_KC12 = A_KC[self.n1:, :self.n1]
            ...
            w = { 
                'A': A_KC[self.n1:, self.n1:].T, 
                'K': np.concatenate( (A_KC12, K2B2), axis=1 ).T,
                'C': Cy2Dy.T
            }
            ...
            if steps_ahead is not None and (len(steps_ahead) > 1 or steps_ahead[0] != 1):
                w['Afw'] = A[self.n1:, self.n1:].T
                if self.nu>0 and need_fw_reg_models:
                    w['Cfw'] = Cy[:, self.n1:].T
                if self.n1 > 0 and (self.nu>0 and self.observable_U_in_Kfw):
                    w['Kfw'] = np.concatenate( (A[self.n1:, :self.n1], B[self.n1:, :]), axis=1 ).T
                elif self.n1 > 0:
                    w['Kfw'] = (A[self.n1:, :self.n1] ).T

            nft2 = self.nu if self.has_UFT else 0
            n1_in = self.n1+self.nu if self.nu > 0 and self.observable_U_in_Kfw else self.n1
            self.model2 = RNNModel(self.n2, self.ny+self.n1+self.nu, self.block_samples, self.batch_size, ny_out=self.ny, nft=nft2, n1_in=n1_in,
                                            has_prior_pred=True, # From stage 1
                                            enable_forward_pred=enable_forward_pred,
                                            steps_ahead=steps_ahead,
                                            observable_U_in_Cfw=self.observable_U_in_Cfw,
    ...
    def find_fixed_points(self, initial_states=[]):
        """Under construction: finds fixed points

        Args:
            initial_states (list, optional): _description_. Defaults to [].

        Returns:
            _type_: _description_
        """        
        if self.n2 > 0:
            logger.warning('WARNING: Not considering stage 2')

        initial_states = [x0[:self.n1] for x0 in initial_states]
        fixed_points = self.model1.find_fixed_points(initial_states=initial_states)
        return fixed_points
    ...
    def predict(self, Y, U=None, x0=None):
        ...
        Returns:
            allZp (np.array): prediction of the output signal z. A list each coontaining prediction for one desired step-ahead
            allYp (np.array), self-prediction of the input signal y. A list each coontaining prediction for one desired step-ahead
            allXp (np.array): estimated latent states. A list each coontaining prediction for one desired step-ahead
        """        
        eagerly_flag_backup = set_global_tf_eagerly_flag(False)
        if hasattr(self, 'yPrepModel') and self.yPrepModel is not None:
            Y = np.array( self.yPrepModel.predict(Y) )

        if hasattr(self, 'YPrepMap') and self.YPrepMap is not None and Y is not None:
        ...
        if not hasattr(self, 'has_UFT'):
            self.has_UFT = True 
        if not hasattr(self, 'has_UFT_reg'):
            self.has_UFT_reg = True 
        if not hasattr(self, 'observable_U_in_Cfw'):
            self.observable_U_in_Cfw = False
        if not hasattr(self, 'observable_U_in_Kfw'):
            self.observable_U_in_Kfw = False
        ...
        self.observable_U_in_Kfw = U is not None and self.nu>0 and self.observable_U_in_Kfw
        self.observable_U_in_Cfw = U is not None and self.nu>0 and self.observable_U_in_Cfw

        need_Cfw = np.any(np.array(steps_ahead)!=1) and ((self.nu > 0 and self.has_UFT and not self.observable_U_in_Cfw) or self.has_Dyz)
        need_fw_reg_models = np.any(np.array(steps_ahead)!=1) and ((self.nu > 0 and self.has_UFT_reg and not self.observable_U_in_Cfw) or self.has_Dyz)
        
        multi_step_with_data_gen = hasattr(self, 'multi_step_with_data_gen') and self.multi_step_with_data_gen
        if multi_step_with_data_gen:
            ...
            steps_ahead, _, steps_ahead_model1, _, model1_orig_step_inds = self.get_model_steps_ahead([1])
        ...
        if U is not None and Y is not None:
            ...
            YU = U
        if self.n1 > 0:
            if self.has_Dyz and self.has_UFT:
                FT_in = YU.T
            elif self.has_UFT: 
                FT_in = UT
            elif self.has_Dyz:
            ...
            if x0 is not None:
                initial_state = x0[:self.n1].T
            else:
                initial_state = None
            preds1 = self.model1.predict(YU.T, FT_in=FT_in, n1_in=n1_in1, initial_state=initial_state, return_internal_states=multi_step_with_data_gen) # Supported of n1_in (U) in stage 1
            allXp1_steps = preds1[:len(steps_ahead_model1)]# if steps_ahead is not None else preds1[0]
            allZp1_steps = preds1[len(steps_ahead_model1):2*len(steps_ahead_model1)]# if steps_ahead is not None else preds1[1]
            if multi_step_with_data_gen:
                allXp1_internal_steps = preds1[3*len(steps_ahead_model1):4*len(steps_ahead_model1)] # Useful for LSTM
            
            allXp1step, allZp1step = allXp1_steps[0], allZp1_steps[0] # This will be used for the next stage

            allZp_steps = [None for s in steps_ahead]
            for saInd in range(len(steps_ahead)):
                allXp_steps[saInd][:, :self.n1] = allXp1_steps[model1_orig_step_inds[saInd]].T
                allZp_steps[saInd] = allZp1_steps[model1_orig_step_inds[saInd]]
                if multi_step_with_data_gen:
                    allXp_internal_steps[saInd] = allXp1_internal_steps[model1_orig_step_inds[saInd]].T            
                ...
                if self.model1_Cy is not None:
                    for saInd in range(len(steps_ahead)):
                        ...
                        if need_fw_reg_models and sa > 1:
                            ...
                            if U is not None and self.has_UFT_reg: # and hasFT
                                UShift = np.concatenate( (U, np.nan*np.ones((sa-1, U.shape[-1]))), axis=0 )[(sa-1):, :]
        ...
        if self.n2 > 0:
            ...
            if x0 is not None:
                ...
                initial_state = None
            if self.n1 > 0 and U is not None and (self.nu>0 and self.observable_U_in_Kfw): # supported of observed U
                allXp1U_steps = []
                for allXp1_this_step, UReverseShifted_this_step in zip(allXp1_steps, n1_in1):
                    allXp1U_this_step = np.concatenate( (allXp1_this_step, UReverseShifted_this_step) )
                    allXp1U_steps.append(allXp1U_this_step)
                n1_in = allXp1U_steps
            elif self.n1 > 0:
            ...
            # FT_in2 = UT if self.has_UFT else None
            if not self.observable_U_in_Cfw:
            ...
            preds2 = self.model2.predict(Y_in_resU, FT_in=FT_in2, n1_in=n1_in, prior_pred=allYp_steps, initial_state=initial_state, prior_pred_shift_by_one=True, return_internal_states=multi_step_with_data_gen)

            allXp2_steps = preds2[:len(steps_ahead)]
            allYpF_steps = preds2[len(steps_ahead):2*len(steps_ahead)]
            if multi_step_with_data_gen:
                allXp2_internal_steps = preds2[3*len(steps_ahead):4*len(steps_ahead)] # Useful for LSTM
           
            allXp2, allYpF = allXp2_steps[0], allYpF_steps[0]

            for saInd in range(len(steps_ahead)):
                allXp_steps[saInd][:, self.n1:] = allXp2_steps[saInd].T
                if multi_step_with_data_gen:
                    if allXp_internal_steps[saInd] is None:
                        allXp_internal_steps[saInd] = allXp2_internal_steps[saInd].T
                    else:
                        allXp_internal_steps[saInd] = np.concatenate((allXp_internal_steps[saInd], allXp2_internal_steps[saInd].T), axis=1)

            allYp_steps = list(allYpF_steps)

        if self.nz > 0 and self.model2_Cz is not None:
            if self.has_Dyz and (self.n1 == 0 or self.model2_Cz_Full) and self.has_UFT_reg:
                UOrYU = YU
            elif self.has_Dyz and (self.n1 == 0 or self.model2_Cz_Full):
                UOrYU = Y
            elif self.has_UFT_reg:
                UOrYU = U
            else:
                UOrYU = None
            for saInd, allXp_this_step in enumerate(allXp_steps):
                ...
                if UOrYU is not None:
                    UOrYUShift = np.concatenate( (UOrYU, np.nan*np.ones((sa-1, UOrYU.shape[-1]))), axis=0 )[(sa-1):, :]
                if not self.model2_Cz_Full:
                    allXp2_this_step = allXp2_steps[saInd]
                    allZp_this_step = allZp_steps[saInd]
                    if need_fw_reg_models and sa > 1:
                        allZpF = self.model2_Cz_fw.predict(allXp2_this_step, prior_pred=allZp_this_step)
                    else:
                        if UOrYU is not None:
                            allXp2U = np.concatenate( (allXp2_this_step.T, UOrYUShift), axis=1 )
                        else:
                            allXp2U = allXp2_this_step.T
                        allZpF = self.model2_Cz.predict(allXp2U.T, prior_pred=allZp_this_step)
                else:
                    if need_fw_reg_models and sa > 1:
                        allZpF = self.model2_Cz_fw.predict(allXp_this_step.T, prior_pred=None)
                    else:
                        if UOrYU is not None:
                            allXpU = np.concatenate( (allXp_this_step, UOrYUShift), axis=1 )
                        else:
        ...
        if self.model1_Cy is not None and self.model1_Cy_Full:
            for saInd, allXp_this_step in enumerate(allXp_steps):
                sa = steps_ahead[saInd]
                if need_fw_reg_models and sa > 1:
                    allYpF = self.model1_Cy_fw.predict(allXp_this_step.T, prior_pred=None)
                else:
                    if U is not None and self.has_UFT_reg:
                        UShift = np.concatenate( (U, np.nan*np.ones((sa-1, U.shape[-1]))), axis=0 )[(sa-1):, :]
                        allXpU = np.concatenate( (allXp_this_step, UShift), axis=1 )
                    else:
                        allXpU = allXp_this_step
                    allYpF = self.model1_Cy.predict(allXpU.T, prior_pred=None)
        ...
        if multi_step_with_data_gen: # The second approach for forecasting
            ...
            # else:
            #     FT_gen_steps = None
            noise_samples = self.multi_step_data_gen_noise_samples if hasattr(self, 'multi_step_data_gen_noise_samples') else 0
            if noise_samples > 0:
                allXp_steps_ns, allYp_steps_ns, allZp_steps_ns = [], [], []
                for nsi in range(noise_samples):
                    allXp_steps_this, allYp_steps_this, allZp_steps_this = self.propagate_prediction_forward_with_data_gen(allXp_steps, allXp_internal_steps, allYp_steps, allZp_steps, steps_ahead_model1_backup, steps_ahead_backup,
                        noise_scale = 0.1 if nsi > 0 else 0, allU_steps=allU_steps)
                    allXp_steps_ns.append(allXp_steps_this)
                    allYp_steps_ns.append(allYp_steps_this)
                    allZp_steps_ns.append(allZp_steps_this)
                logger.info(f'Averaging BRAID forecasting predictions across {noise_samples} noise realizations')
                ...
                allXp_steps, allYp_steps, allZp_steps = self.propagate_prediction_forward_with_data_gen(allXp_steps, allXp_internal_steps, allYp_steps, allZp_steps, steps_ahead_model1_backup, steps_ahead_backup, allU_steps=allU_steps)
    ...
    def propagate_prediction_forward_with_data_gen(self, allXp_steps, allXp_internal_steps, allYp_steps, allZp_steps, steps_ahead_model1, steps_ahead, noise_scale=0, allU_steps=None):
        # Pass current predictions as new observations and do one more recursion from both stages
        # Note: This version assumes the status of observing U in forecasting is the same as in 1-step (both K and C):
        # Since this implementation does not rely on learning extra params i.e., Afw, Kfw, Cfw, model1_Cy_fw, model2_Cz_fw:
        # Note 1: This version assumes the status of feedtrough in forecasting is the same as in 1-step (makes sure Cfw not used even if it exists in the model).
        # Note 2: This version assumes the status of -U as input to K()- in forecasting is the same as in 1-step (makes sure U is oberved for forecasting as long as it was modeled in 1-step).
        allXp_all_steps, allYp_all_steps, allZp_all_steps = [], [], []
        # need_fw_reg_models = np.any(np.array(steps_ahead)!=1) and ((self.nu > 0 and self.has_UFT_reg and not self.observable_U_in_Cfw) or self.has_Dyz)
        for step_ahead in steps_ahead_model1:
            if step_ahead == 1: # get 1-step ahead preds that are already computed in MainModel.predict earlier
                next_states = np.array(allXp_steps[0]) # sample * dim
                ...
                n1_internal = 2*self.n1 if self.n1 > 0 and self.model1.LSTM_cell else self.n1
                n2_internal = 2*self.n2 if self.n2 > 0 and self.model2.LSTM_cell else self.n2
                next_Yp = np.zeros_like(latest_Yp)
                next_Zp = np.zeros_like(latest_Zp)
                if allU_steps is not None and self.nu > 0: # Let's by default observe the U even if Kfw is not learned (not optimized with observable U in Kfw) for.  => commented: and self.observable_U_in_Kfw
                    latest_YpU = np.concatenate( (latest_Yp, allU_steps[step_ahead-2].T), axis=1 )
                else:
                    latest_YpU = latest_Yp
                if self.has_UFT and allU_steps is not None and self.nu > 0: #and self.observable_U_in_Cfw: commented the last condition to make U observable in forecasting by default (As long as the original C supports it)
                    FT_this = allU_steps[step_ahead-1].T
                else:
                    FT_this = None
                if self.n1 > 0:
                    next_states1, next_states_internal1, next_Zp1 = self.model1.rnn.cell.applyRecursionOnMany(
                            latest_states_internal[:, :n1_internal], 
                            latest_YpU,
                            ft_at_t_plus_1=FT_this)
                    ...
                    if allU_steps is not None and self.has_UFT_reg and self.nu > 0:
                        Xp1U = np.concatenate( (next_states1, allU_steps[step_ahead-1].T) , axis=1)
                    else:
                        Xp1U = next_states1
                    next_Yp1 = self.model1_Cy.predict(Xp1U.T).T

                    # next_Yp1 = self.model1_Cy.predict(next_states1.T).T
                    next_states[:, :self.n1] = np.array(next_states1)
                    next_states_internal[:, :n1_internal] = np.array(next_states_internal1)
                    next_Yp += next_Yp1
                    next_Zp += next_Zp1
                if self.n2 > 0:
                    if self.n1 > 0:
                        ...
                        prior_Yp = None
                    if allU_steps is not None and self.nu > 0: # Let's by default observe the U even if Kfw is not learned (not optimized with observable U in Kfw) for => commented: and self.observable_U_in_Kfw 
                        RNN2Input = np.concatenate( (latest_states[:, :self.n1], latest_Yp,  allU_steps[step_ahead-2].T), axis=1 )
                    else:
                        RNN2Input = np.concatenate( (latest_states[:, :self.n1], latest_Yp), axis=1 )
                    next_states2, next_states_internal2, next_Yp2 = self.model2.rnn.cell.applyRecursionOnMany(
                            latest_states_internal[:, n1_internal:], 
                            RNN2Input, 
                            ft_at_t_plus_1=FT_this,
                            input_at_t_prior_pred=prior_Yp)
                    next_states[:, self.n1:] = np.array(next_states2)
                    next_states_internal[:, n1_internal:] = np.array(next_states_internal2)
                    ...
                    if self.model2_Cz is not None and not self.model2_Cz_Full:
                        if allU_steps is not None and self.has_UFT_reg and self.nu > 0: # and hasFT
                            Xp2U = np.concatenate( (next_states[:, self.n1:], allU_steps[step_ahead-1].T) , axis=1)
                        else:
                            Xp2U = next_states[:, self.n1:]
                        next_Zp = self.model2_Cz.predict(Xp2U.T, prior_pred=prior_ZpT).T
                        # next_Zp = self.model2_Cz.predict(next_states[:, self.n1:].T, prior_pred=prior_ZpT).T
                if self.model1_Cy is not None and self.model1_Cy_Full:
                    if allU_steps is not None and self.has_UFT_reg and self.nu > 0: # and hasFT
                        XpU = np.concatenate( (next_states, allU_steps[step_ahead-1].T) , axis=1)
                    else:
                        XpU = next_states.T
                    next_Yp = self.model1_Cy.predict(XpU.T).T
                    # next_Yp = np.array(self.model1_Cy.predict(next_states.T).T)
                if self.model2_Cz is not None and self.model2_Cz_Full:
                    if allU_steps is not None and self.has_UFT_reg and self.nu > 0: # and hasFT
                        XpU = np.concatenate( (next_states, allU_steps[step_ahead-1].T),  axis=1 )
                    else:
                        XpU = next_states
                    next_Zp = self.model2_Cz.predict(XpU.T).T
    ...
    def generateRealization(self, N, random_x0=False, x0=None, u=None, eY=None, eZ=None):
        """Generates a random realization based on the MainModel
        ...
        if random_x0 and hasattr(self, 'XCov') and not np.any(np.isnan(self.XCov)):
            x0 = np.atleast_2d(np.random.multivariate_normal(mean=np.zeros(self.nx), cov=self.XCov)).T
        X = np.empty((N, self.nx)); X[:] = np.NaN
        Y = np.empty((N, self.ny)); Y[:] = np.NaN
        Z = np.empty((N, self.nz)); Z[:] = np.NaN
        if eY is None:
        ...
        for i in range(N):
            ...
            if self.model1_Cy_Full:
                yk_res += self.model1_Cy.predict(xk_uk)
            else:
                if self.n1 > 0:
            ...
            if self.n1 > 0:
                Xp1_cur, zk_next1, Xp1_next = self.model1.predict(yk_uk, initial_state=Xp1.T, FT_in=ftk)
            else:
                Xp1_next = np.zeros(self.n1)
            Xp1_next = Xp1_next[:, np.newaxis]

            if self.n2 > 0:
                Xp2_cur, yk_next1, Xp2_next = self.model2.predict(x1_yk_uk, initial_state=Xp2.T, n1_in=Xp1_next, FT_in=ftk)
